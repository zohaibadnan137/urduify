...
def Roman_to_urdu(sentence):
    ...
    for i in range(len(words)):
        # If the Roman Urdu word exists in the dictionary, append its Urdu counterpart
        if words[i].lower() in map_dict.keys():
            urdu_list.append(map_dict[words[i].lower()][0])
        # Otherwise, use the Google Translate library
        else:
            val = ts.google(words[i].lower(), to_language='ur')
            urdu_list.append(val)
